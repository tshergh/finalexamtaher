    ...
    try:
        ...
        if nabilanavab == False:
            ...
            downloadMessage = await askPDF.reply_text(
                                                     f"`مجموع  Total PDF's : {len(MERGE[chat_id])}`.. 💡",
                                                     quote = True
                                                     )
            asyncio.sleep(.5); i = 0
            ...
            for iD in MERGE[chat_id]:
                await downloadMessage.edit(
                                          f"__بدء تنزيل ملف PDF \nStarted Downloading Pdf :{i+1} 📥__"
                                          )
                # START DOWNLOAD
                c_time = time.time()
            ...
            output_pdf = f"merge{chat_id}/مدمج.pdf"
            #PyPDF 2
