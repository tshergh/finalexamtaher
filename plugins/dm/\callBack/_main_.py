...
async def _pdf(bot, callbackQuery):
    try:
        ...
        if callbackQuery.data.startswith("KpdfInfo"):
            _, number_of_pages = callbackQuery.data.split("|")
            return await callbackQuery.answer(
                                      "♎ مجموع TOTAL {} PAGES الصفحات ♎".format(number_of_pages)
                                      )
        ...
        if data.startswith(tuple(["decrypt", "Kdecrypt", "encrypt", "Kencrypt"])):
            ...
            password = await bot.ask(
                                    ...
                                    text = f"__PDF {work} »\n"
                                            "الان ، أدخل كلمة المرور \nNow, please enter the password :__\n\n"
                                            "/exit __ لإلغاء to cancel__",
        ...
        downloadMessage = await callbackQuery.message.reply_text(
                                                                "`تنزيل ملفك Downloding your pdf..` 📩", 
        ...
        await downloadMessage.edit(
                                  "⚙️ ` بدات العلمية Started Processing.. \nانتظر بعض وقت It might take some time..`💛",
                                  reply_markup = cancelBtn
                                  )
        ...
        if (data[0] != 'K') or not (data in ["rot180", "rot90", "rot270"]):
            ...
            if data.startswith("decrypt"):
                if not(checked == "encrypted"):
                    await downloadMessage.edit(
                                              "`ملف ليس مشفرFile Not Encrypted..`🙏🏻"
                                              )
        ...
        if chat_id in PROCESS:
            ...
            if data.startswith(tuple(["ocr", "Kocr"])):
                if number_of_pages>5:
                    await downloadMessage.edit(
                                              "__ارسل ملف اقل من ٥ صفحات\nSend me a file less than 5 images__ 😅"
                                              )
                    PROCESS.remove(chat_id)
                    ...
                    await downloadMessage.edit(
                                              "⚙️ `إضافة طبقة Adding OCR Layer.. ✍️\nقد يستغرق بعض وقت\n It might take some time..`💛",
                                              reply_markup = cancelBtn
                                              )
        ...
        await downloadMessage.edit(
                                  "⚙️ `بدا التحميل Started Uploading..` 📤",
                                  reply_markup = cancelBtn
                                  )
