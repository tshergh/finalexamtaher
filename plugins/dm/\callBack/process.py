...
passwordError = "__لا يمكن فك تشفير الملف بامتداد\nCannot Decrypt the file with__ `{}` 🕸️"

decrypted = "__ملف مفكك\nDecrypted File__"

encryptedFileCaption = "__Page Number(عدد صفحات)__: {}\n__key(المفتاح)__ 🔐: ||{}||"

compressedCaption = """`Original Size(الحجم الأصلي) : {}
\n
Compressed Size(الحجم المضغوط) : {}
\n
Ratio(النسبة) : {:.2f} %`"""

cantCompressMore = "لا يمكن ضغط الملف أكثر ..🤐\nFile Can't be Compressed More..🤐"

pgNoError = """__بالنسبة لبعض الأسباب ، يدعم تنسيق A4 ملفات PDF التي تحتوي على أقل من 5 صفحات\nFor Some Reason A4 FORMATTING Supports for pdfs with less than 5 Pages__"
...
async def encryptPDF(message_id, password):
    try:
        ...
        logger.exception(
                        "ENCRYPT[PROCESS]:CAUSES %(e)s ERROR خطا",
                        exc_info=True
                        )
        return False
...
async def compressPDF(message, message_id):
    try:
        ...
        await message.edit(
                          f"❌شيء خاطئSOMETHING WENT WRONG❌"
                          f"\n\nError: {e}"
                          )
        return False
...
async def ocrPDF(message, message_id):
    try:
        try:
            ...
            await message.edit(
                              "`لديك بالفعل طبقة نصيةAlready Have A Text Layer.. `😏"
                              )
            return False
    except Exception as e:
        logger.exception(
...
async def rotatePDF(data, message_id):
    try:
        ...
        if data == "rot90":
            ...
            caption = "__استدارةRotated 90°__"
        # ROTATE 180°
        if data == "rot180":
            ...
            for i in range(number_of_pages):
                ...
                pdf_writer.addPage(page)
            caption = "__استدارةRotated: 180°__"
        # ROTATE 270°
        if data == "rot270":
            ...
            for i in range(number_of_pages):
                ...
                pdf_writer.addPage(page)
            caption = "__استدارةRotated: 270°__"
...
async def formatterPDF(message, message_id):
    try:
        ...
        return "__ بتنسيق pdf A4 formatted pdf__"
    except Exception as e:
        logger.exception(
